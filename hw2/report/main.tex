...
Some comments: the dataset is highly imbalanced. More specifically, the least
represented class is the one that represents the command "brake" on the track.
The anatomy of the dataset is the most important problem to solve in this task.
It will be addressed in an appropriate section.

\newpage

\section{Model}

The Model is a CNN, since we have to classify images. The choice is to implement
a pretty standard CNN, with 3 convolutional layers, 3 pooling layers and 3 batch
normalization layers. The used 
